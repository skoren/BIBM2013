...
\title{\emph{De novo} likelihood-based measures for comparing metagenomic assemblies}
...
Atif Memon\IEEEauthorrefmark{2}, \\
Todd Treangen\IEEEauthorrefmark{4} and
...
Traditional \emph{de novo} genome assemblers have trouble assembling these datasets due to the presence of closely related species and distinguishing between true polymorphisms and errors arising from the sequencing technology.
...
Therefore, we now assume that while the abundances of each organism may very dramatically, the sequencing process still has uniform coverage across the \emph{entire} environment.
...
The closer the LAP scores to the correct abundance ratio, the higher the LAP scores with the true abundance ratios yielding the highest LAP scores in both communities.
...
The additional \emph{de novo} and reference-based metrics for the assemblies were taken from ~\cite{treangen2013metamos}.  These metrics include: total number of contigs/scaffolds in the assembly (\#ctgs), fraction of contigs that mapped without errors to reference genomes (Good Ctgs), total amount of sequence (in Mbp) that can be aligned to the reference genomes (Total Aln), slight mis-assemblies determined by alignments that cover 80\% or more of the aligned contig in a single match (Slt), heavy misassemblies determined by alignments that cover less than 80\% of the aligned contig in a single match or have two or more matches to a single reference (Hvy), Chimeras are contigs with matches to two distinct reference genomes (Ch), the size of the largest contig c such that the sum of all contigs larger than c is more than 10 Mbp (similar to the commonly used N50 size) (Size @ 10 Mbp), size of the largest contig in the assembly (Max ctg size), and average number of errors per Mbp (Err per Mbp).
...
It is important to note that if user selected an assembly based on the best contiguity at 10Mbp, they would select the MetaVelvet assembly, which contains more than double the error rate per Mbp as the SOAPdenovo assembly while aligning nearly 2Mbp less to the references.
...
The SOAPdenovo assembly contains a greater number of contigs at a very low abundance (Fig. \ref{fig:coverage}).
...
\caption{Comparison of assembly statistics for HMP mock Even and mock Staggered datasets. Numbers in bold represent the best value for the specific dataset.}
\label{table_example}
\centering
