...
Despite decades of research, assessing and comparing assembly quality still relies on the availability of independently determined standards, such as manually curated genome sequences.
These standards are often not possible in metagenomic projects, where a large number of novel organisms and strains are discovered, thus we must rely on \emph{de novo} methods for assessing and comparing assembly qualities.
Here we describe an extension to our \emph{de novo} LAP framework to evaluate metagenomic assemblies.
We will show that by modifying our likelihood calculation to take into account abundances, we can accurately and efficiently evaluate metagenomic assemblies.
%We evaluate our extended framework on results generated from the Human Microbiome Project (HMP) and 
We find that our extended LAP framework is able to reproduce results generated from the Human Microbiome Project (HMP) that closely match the reference-based evaluation metrics used in the study and outperforms other \emph{de novo} metrics traditionally used to measure assembly quality.
Finally, we have integrated our LAP framework into the metagenomic assembly pipeline MetAMOS, allowing any user to reproduce reference-based quality evaluations of metagenomic assemblies with relative ease.
...
\begin{comment}
\begin{table}[h]
\caption{LAP assembly}
\label{tab:lap_assembly}
\centering
\begin{tabular}{{c}{c}{c}}
\hline
\bfseries Assembly & \bfseries LAP score & \bfseries N50 (kbp)\\
\hline\hline
Reference (true abundances) & -11.327 & 3,977 \\
Reference (no abundances)   & -11.450 & 3,977 \\
Newbler                     & -11.387 & 314\\
\end{tabular}
\end{table}
\end{comment}
...
%We have made a number of simplifying assumptions about the sequencing process throughout our paper.
